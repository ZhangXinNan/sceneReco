...
def read_charset(filename):
    ...
    with open(filename, 'r') as fi:
        for line in fi:
            ...
            charset_map[line] = len(charset_list)
            charset_list.append(line)
    return charset_list, charset_map


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--charset', default='/Users/zhangxin/gitlab/dmocr/data/char6/char_set.txt')
    return parser.parse_args()

def main(args):
    charset_list, charset_map = read_charset(args.charset)
    charset_flag = [None] * len(charset_list)
    for index, a in enumerate(alphabet):
        if a in charset_map:
            charset_flag[charset_map[a]] = index
    for i, c in enumerate(charset_list):
        print i, c, charset_flag[i]
